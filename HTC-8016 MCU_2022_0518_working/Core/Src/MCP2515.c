...
uint8_t	SpiMCPTxBuf[6] = {0,0,0,0,0,0};
...
{
	uint8_t SpiCanRxBuf[1] = {0};
  
	MCP2515_CS_LOW();

	SpiMCPTxBuf[0] = MCP2515_READ;
	SpiMCPTxBuf[1] = address;
	HAL_SPI_Transmit(SPI_CAN, SpiMCPTxBuf,2,100);
	HAL_SPI_Receive(SPI_CAN, SpiCanRxBuf,1,100);	

	MCP2515_CS_HIGH();
	return SpiCanRxBuf[0];
}
...
{    
	...
	SpiMCPTxBuf[0] = MCP2515_WRITE;
	SpiMCPTxBuf[1] = address;
	SpiMCPTxBuf[2] = data;
	HAL_SPI_Transmit(SPI_CAN, SpiMCPTxBuf,3,100);	
